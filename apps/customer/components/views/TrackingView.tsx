...
export function TrackingView({ currentOrder: initialOrder, orderId, onBackHome, t }: TrackingViewProps) {
  ...
  if (!orderId) {
    return (
      <div className="albaz-shell container mx-auto px-4 py-6 max-w-2xl pb-24 flex items-center justify-center min-h-screen">
  ...
  return (
    <div className="albaz-shell container mx-auto px-4 py-6 max-w-2xl pb-24">
      <Card>
        ...
        <CardContent className="space-y-8">
          {currentOrder && (
            <div className="bg-[var(--albaz-olive)]/5 border border-[var(--albaz-olive)]/20 rounded-lg p-4 text-center">
              <p className="text-sm font-semibold text-[var(--albaz-olive)]">{getStatusText(currentOrder.status, t)}</p>
            </div>
          )}
