...
import { useOrderTracking } from "@/app/customer/hooks/use-order-tracking"
...
const currency = "DZD"
const paymentsEnabled = process.env.NEXT_PUBLIC_ENABLE_STRIPE === 'true'
...
export function CheckoutPage({ order }: CheckoutPageProps) {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (paymentsEnabled && (!stripe || !elements)) return
    ...
    if (!paymentsEnabled) {
      ...
      toast({
        ...
        description: "La commande est enregistrée. Le paiement se fera hors ligne pour l’instant.",
      })
      return
    }
  ...
  const isPayDisabled =
    ...
    (paymentsEnabled && (!stripe || !cardComplete)) ||
    isLoading ||
    zoneStatus === "invalid" ||
                ...
                {tracking?.order && (
                  <div className="space-y-2 rounded-md border p-3">
                    <div className="flex justify-between items-center">
                      ...
                      <Badge variant="outline">{tracking.order.status}</Badge>
                    </div>
                    {tracking.tracking?.estimatedDeliveryTime && (
                      <div className="flex justify-between text-sm">
                        <span>ETA</span>
                        <span>{tracking.tracking.estimatedDeliveryTime} min</span>
                      </div>
                    )}
                    {tracking.tracking?.driver && (
                      <div className="text-sm text-muted-foreground">
                        Livreur: {tracking.tracking.driver.name} ({tracking.tracking.driver.phone || "N/A"})
                      </div>
                    )}
                    {tracking.tracking?.lastUpdated && (
                      <div className="text-xs text-muted-foreground">
                        Dernière mise à jour: {new Date(tracking.tracking.lastUpdated).toLocaleTimeString()}
                      </div>
                    )}
                    {tracking.message && (
                      <p className="text-xs text-muted-foreground">{tracking.message}</p>
                    )}
                  </div>
                )}
              </CardContent>
