...
// Detect Electron runtime via preload bridge exposed on window
const [isElectronRuntime, setIsElectronRuntime] = useState(false)
useEffect(() => {
  try {
    // `window.electronAPI` is exposed by the Electron preload script
    const win = window as any
    setIsElectronRuntime(Boolean(win?.electronAPI?.isElectron))
  } catch (e) {
...
const saveSettings = () => {
  try {
    localStorage.setItem('vendor_language', language)
    localStorage.setItem('vendor_dark', String(isDarkMode))
    if (coverPhoto) localStorage.setItem('vendor_cover', coverPhoto)
  } catch (e) {
    console.warn('Failed to save settings', e)
  }
}

// Settings: cover photo + persisted preferences
const [coverPhoto, setCoverPhoto] = useState<string | null>(null)
useEffect(() => {
  try {
    const savedLang = localStorage.getItem('vendor_language')
    const savedTheme = localStorage.getItem('vendor_dark')
    const savedCover = localStorage.getItem('vendor_cover')
...
useEffect(() => {
  ...
  const completeSale = async (paymentMethod: "cash" | "card") => {
    ...
    // Calculate values completely inline - ABSOLUTELY NO const/let/var declarations
    // All computations happen directly in expressions to prevent webpack hoisting
    try {
      ...
      const response = await fetch(salesUrl, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        // Compute ALL values inline - no variables whatsoever
        body: JSON.stringify({
  ...
  // Note: do not include `handleDataLoad` in deps to avoid re-running if its
  // identity changes (it is already memoized). Depend only on the actual
  // context values that should trigger a reload.
  useEffect(() => {
    ...
    // Intentionally exclude `handleDataLoad` from deps
  }, [status, isAuthenticated, isAdmin, selectedVendorId])
          ...
          <div className="flex gap-6">
            <aside className="w-64">
              ...
              <div className="mb-4 sticky top-6">
                {coverPhoto ? (
                  <div className="px-3">
                    <img src={coverPhoto} alt="Cover" className="w-full h-28 object-cover rounded-lg shadow-sm" />
                  </div>
                ) : (
                  <div className="px-3">
                    <div className="w-full h-28 rounded-lg bg-gradient-to-br from-teal-500 via-cyan-400 to-orange-500 flex items-center justify-center shadow-md">
                      <img src="/logo.png" alt="AL-baz" className="h-10" />
                    </div>
                  </div>
                )}
              </div>
              <TabsList className="flex flex-col space-y-2 sticky top-6">
            ...
            <TabsTrigger value="ai">
              ...
              <TabsTrigger value="settings">
                ...
                {translate('Paramètres','الإعدادات')}
              </TabsTrigger>
              </TabsList>
            </aside>
              ...
              <CardContent className="p-6">
                {customers.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-lg text-muted-foreground mb-4">
                      {translate("Aucun client trouvé", "لا يوجد عملاء")}
                    </p>
                    <Button onClick={() => setShowCustomerDialog(true)}>
                      <Plus className={`w-4 h-4 ${isArabic ? "ml-2" : "mr-2"}`} />
                      {translate("Ajouter votre premier client", "أضف عميلك الأول")}
                    </Button>
                  </div>
                ) : (
                  <Table>
              ...
              <CardContent>
                <div className="space-y-4 max-w-lg">
                  ...
                  <div>
                    ...
                    {coverPhoto && (
                      <div className="mt-2">
                        <img src={coverPhoto} alt="Cover" className="w-48 h-24 object-cover rounded" />
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button onClick={() => { saveSettings(); toast({ title: 'Paramètres enregistrés' }) }}>Enregistrer</Button>
                  </div>
                </div>
              </CardContent>
            </Card>
