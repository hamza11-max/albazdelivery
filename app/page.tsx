...
export default function AlBazApp() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem('albaz-language', selectedLanguage)
    }
    ...
    const fetchAds = async () => {
      try {
        ...
        if (data.success && data.ads) {
          setAds(data.ads.filter((ad: any) => ad.isActive))
        }
      } catch (error) {
          ...
          <CardContent className="space-y-1">
            ...
            <button
              onClick={() => {
                ...
                if (typeof window !== 'undefined') {
                  localStorage.setItem('albaz-theme', newMode ? 'dark' : 'light')
                }
              }}
              className="w-full flex items-center justify-between p-4 hover:bg-muted rounded-lg transition-colors"
            >
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  <p className="font-semibold text-foreground">
                    {t("theme", "Thème", "المظهر")}
                  </p>
                  <p className="text-sm text-muted-foreground">
                    {isDarkMode ? t("dark-mode", "Mode sombre", "الوضع الداكن") : t("light-mode", "Mode clair", "الوضع الفاتح")}
                  </p>
                </div>
              </div>
              <ChevronRight className="w-5 h-5 text-muted-foreground" />
